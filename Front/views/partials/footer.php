	<div class="container footer">
		<div class="content">
			<div class="row">
				<div class="col-md-4 footer_nav">
					<h2 class="footer_title">PremProject</h2>
					<ul>
						<li><a href="<?= URL ?>product">Produit</a></li>
						<li><a href="<?= URL ?>abonnement">Abonnement</a></li>
						<li><a href="<?= URL ?>blog">Blog</a></li>
						<li><a href="<?= URL ?>mentions-legales">Mentions légales</a></li>
						<li><a href="<?= URL ?>cgv">CGV</a></li>
					</ul>
				</div>

				<div class="col-md-4 footer_nav f_center">
					<h2 class="footer_title">Support</h2>
					<ul>
						<li><a href="<?= URL ?>faq">FAQ</a></li>
						<li><a href="<?= URL ?>guide">Guide</a></li>
						<li><a href="<?= URL ?>conseil">Conseil</a></li>
						<li><a href="<?= URL ?>contact">Nous contacter</a></li>
						<li><a href="<?= URL ?>dev">Dévelopeurs</a></li>
					</ul>
				</div>

				<div class="col-md-4 footer_nav f_right">
					<h2 class="footer_title">Nous suivre</h2>
					<ul>
						<li><a href="<?= $navFacebook ?>" target="_blank">Facebook</a></li>
						<li><a href="<?= $navTwitter ?>" target="_blank">Twitter</a></li>
						<li><a href="<?= $navYoutube ?>" target="_blank">Youtube</a></li>
					</ul>
				</div>
			</div>
		</div>
	</div>

	<div class="container">
		<div class="row">
			<div class=".col-md-12 footer_copyright">
				copyright PremProject | 2016
			</div>
		</div>
	</div>

    <script src="<?= URL ?>src/js/libs/jquery.min.js"></script>
    <script src="<?= URL ?>src/js/libs/youtube_iframe.js"></script>
	<script src="<?= URL ?>src/js/libs/bootstrap.min.js" integrity="sha384-0mSbJDEHialfmuBBQP6A4Qrprq5OVfW37PRR3j5ELqxss1yVqOtnepnHVP9aJ7xS" crossorigin="anonymous"></script>
</body>
</html>